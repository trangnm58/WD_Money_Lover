var checkEmptyList = function() {
	if ($("wallet-container > a").length == 0) {
		$("wallet-container").append('<span>No transaction. Add more!</span>');
	} else {
...
function addWallet() {
    ...
    type = $("#type").val();    
	$.post("/api/add-wallet", {type: type, name: name, amount: amount, unit: unit, description: description}, function(data, status) {		
		$("#wallet-container").append(
		'<a class="list-group-item total each-wallet" id ="' + data["id"] + '"  data-toggle="modal" data-target="#manageWallet"> <i class=" fa ' + type +  ' fa-2x " aria-hidden="true" > </i> <div class="each-wallet-info"> <strong>' + name + '</strong> <p>' + unit_name +' '+ amount+ ' </p>  </div> </a>'
		);
    });
	backToList();
	return false;
...
function saveEditWallet() {
    ...
    data = {name: name, amount: amount, unit_id:unit_id, description:description, type:type};
